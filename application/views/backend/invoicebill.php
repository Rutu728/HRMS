...
<?php $this->load->view('backend/sidebar'); ?> 
         ...
         <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-themecolor"><i class="fa fa-compass" style="color:#1976d2"></i> Invoice</h3>
                </div>
            ...
            <div class="container-fluid">
                ...
                <div class="row m-b-10"> 
                    <div class="col-12">
                        <button type="button" class="btn btn-info"><i class="fa fa-plus"></i><a data-toggle="modal" data-target="#exampleModal" data-whatever="@getbootstrap"  class="text-white"><i class="" aria-hidden="true"></i> Add Invoice </a></button>
                        <button type="button" class="btn btn-primary"><i class="fa fa-bars"></i><a href="<?php echo base_url(); ?>invoicebill/invoicebill" class="text-white"><i class="" aria-hidden="true"></i> Invoice List</a></button>
                    </div>
                </div>         
                <div class="row">
                    <div class="col-12">
                            ...
                            <div class="card-body">
                                <div class="table-responsive ">
                                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                           <?php foreach($invoicebill as $value): ?>
                                            <tr>
                                    ...
                                    <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel1">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content ">
                                                ...
                                                <form method="post" action="Add_invoicebill" id="btnSubmit" enctype="multipart/form-data">
   ...
   $(document).ready(function () {
    ...
    <div style="font-family: Arial, sans-serif; margin: 0 auto; max-width: 700px; height:90vh; padding: 20px; border: 1px solid #ccc; border-radius: 5px; background-color: #f9f9f9;">
                ...
                <div style="display:flex; align-items:center; justify-content:space-between;"> <h2 style="font-size: 24px;">Invoice Details</h2> <img src="<?php echo base_url(); ?>assets/images/mh.png" style="max-width: 150px; margin-top:-25px;"></div>  
        ...
        <p ><strong>Invoice ID:</strong> ${response.invoicebill.id}</p>
    ...
    <thead>
        <tr>
        <th style="border-bottom: 1px solid #ccc; border-top: 1px solid #ccc; border-right: 1px solid #ccc; border-left: 1px solid #ccc; padding: 8px; text-align: center;">Customer Name</th>
    ...
    <tbody>
        ...
        <tr>
        <td style="border-bottom: 1px solid #ccc; border-right: 1px solid #ccc; border-left: 1px solid #ccc; padding: 8px; text-align: center;">${response.invoicebill.cust_name}</td>
            <td style="border-bottom: 1px solid #ccc; border-right: 1px solid #ccc; border-left: 1px solid #ccc; padding: 8px; text-align: center;">${response.invoicebill.item_name}</td>
            <td style="border-bottom: 1px solid #ccc; border-right: 1px solid #ccc; padding: 8px; text-align: center;">₹ ${response.invoicebill.price}</td>
            <td style="border-bottom: 1px solid #ccc; border-right: 1px solid #ccc; padding: 8px; text-align: center;">${response.invoicebill.quantity}</td>
            <td style="border-bottom: 1px solid #ccc; border-right: 1px solid #ccc; padding: 8px; text-align: center;">₹ ${response.invoicebill.tax}</td>
            <td style="border-bottom: 1px solid #ccc; border-right: 1px solid #ccc; padding: 8px; text-align: center;">₹ ${response.invoicebill.total}</td>
        </tr>
        
    </tbody>
</table>

<p style="text-align:right; margin-top:60px;"><strong>Subtotal :- </strong>₹ ${response.invoicebill.subtotal}</p>
<p style="text-align:right;"><strong>Payment Mode :-</strong> ${response.invoicebill.type}</p>
                                        ...
                                        $(document).ready(function () {
                                            $(".disiplinary").click(function (e) {
                                                ...
                                                $.ajax({
													...
													$('#btnSubmit').find('[name="id"]').val(response.invoicebill.id).end();
                                                    $('#btnSubmit').find('[name="cust_name"]').val(response.invoicebill.cust_name).end();
                                                    $('#btnSubmit').find('[name="item_name"]').val(response.invoicebill.item_name).end();
                                                    $('#btnSubmit').find('[name="quantity"]').val(response.invoicebill.quantity).end();
                                                    $('#btnSubmit').find('[name="price"]').val(response.invoicebill.price).end();
                                                    $('#btnSubmit').find('[name="tax"]').val(response.invoicebill.tax).end();
                                                    $('#btnSubmit').find('[name="total"]').val(response.invoicebill.total).end();
                                                    $('#btnSubmit').find('[name="subtotal"]').val(response.invoicebill.subtotal).end();
                                                    $('#btnSubmit').find('[name="type"]').val(response.invoicebill.type).end();
                                                    $('#btnSubmit').find('[name="date"]').val(formatDate(response.invoicebill.date)).end();
												});
                                            });
                                        });
...
function confirmDelete(id) {
				if (confirm('Are you sure to delete this value?')) {
					window.location.href = 'DeletInvoicebill?D=' + id;
				}
