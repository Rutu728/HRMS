...
<?php $this->load->view('backend/sidebar'); ?> 
         ...
         <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-themecolor"><i class="fa fa-compass" style="color:#1976d2"></i> Quotation</h3>
                </div>
            ...
            <div class="container-fluid">
                ...
                <div class="row m-b-10"> 
                    <div class="col-12">
                        <button type="button" class="btn btn-info"><i class="fa fa-plus"></i><a data-toggle="modal" data-target="#exampleModal" data-whatever="@getbootstrap"  class="text-white"><i class="" aria-hidden="true"></i> Add Quotation </a></button>
                        <button type="button" class="btn btn-primary"><i class="fa fa-bars"></i><a href="<?php echo base_url(); ?>invoicebill/invoicebill" class="text-white"><i class="" aria-hidden="true"></i> Quotation List</a></button>
                    </div>
                </div>         
                <div class="row">
                    <div class="col-12">
                                    ...
                                    <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel1">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content ">
                                                ...
                                                <form method="post" action="Add_Quotation" id="btnSubmit" enctype="multipart/form-data">
   ...
   $(document).ready(function () {
    ...
    <div style="font-family: Arial, sans-serif; margin: 0 auto; max-width: 700px; height:90vh; padding: 20px; border: 1px solid #ccc; border-radius: 5px; background-color: #f9f9f9;">
                ...
                <div style="display:flex; align-items:center; justify-content:space-between;"> <h2 style="font-size: 24px;">Quotation Details</h2> <img src="<?php echo base_url(); ?>assets/images/mh.png" style="max-width: 150px; margin-top:-25px;"></div>  
        ...
        <p ><strong>Quotation ID:</strong> ${response.quotation.id}</p>
    ...
    <tbody>
        ...
        <tr>
        <td style="border-bottom: 1px solid #ccc; border-right: 1px solid #ccc; border-left: 1px solid #ccc; padding: 8px; text-align: center;">${response.quotation.cust_name}</td>
            <td style="border-bottom: 1px solid #ccc; border-right: 1px solid #ccc; border-left: 1px solid #ccc; padding: 8px; text-align: center;">${response.quotation.item_name}</td>
            <td style="border-bottom: 1px solid #ccc; border-right: 1px solid #ccc; padding: 8px; text-align: center;">₹ ${response.quotation.price}</td>
            <td style="border-bottom: 1px solid #ccc; border-right: 1px solid #ccc; padding: 8px; text-align: center;">${response.quotation.quantity}</td>
            <td style="border-bottom: 1px solid #ccc; border-right: 1px solid #ccc; padding: 8px; text-align: center;">₹ ${response.quotation.tax}</td>
            <td style="border-bottom: 1px solid #ccc; border-right: 1px solid #ccc; padding: 8px; text-align: center;">₹ ${response.quotation.total}</td>
        </tr>
        
    </tbody>
</table>

<p style="text-align:right; margin-top:60px;"><strong>Subtotal :- </strong>₹ ${response.quotation.subtotal}</p>
                                        ...
                                        $(document).ready(function () {
                                            $(".disiplinary").click(function (e) {
                                                ...
                                                $.ajax({
													...
													$('#btnSubmit').find('[name="id"]').val(response.quotation.id).end();
                                                    $('#btnSubmit').find('[name="item_name"]').val(response.quotation.item_name).end();
                                                    $('#btnSubmit').find('[name="cust_name"]').val(response.quotation.cust_name).end();
                                                    $('#btnSubmit').find('[name="quantity"]').val(response.quotation.quantity).end();
                                                    $('#btnSubmit').find('[name="price"]').val(response.quotation.price).end();
                                                    $('#btnSubmit').find('[name="tax"]').val(response.quotation.tax).end();
                                                    $('#btnSubmit').find('[name="total"]').val(response.quotation.total).end();
                                                    $('#btnSubmit').find('[name="subtotal"]').val(response.quotation.subtotal).end();
                                                    $('#btnSubmit').find('[name="type"]').val(response.quotation.type).end();
                                                    $('#btnSubmit').find('[name="date"]').val(formatDate(response.quotation.date)).end();
												});
                                            });
                                        });
...
function confirmDelete(id) {
				if (confirm('Are you sure to delete this value?')) {
					window.location.href = 'DeletQuotation?D=' + id;
				}
