...
class Incident extends CI_Controller {
    ...
    public function Incident(){
        ...
        $data['incident'] = $this->incident_model->incidentfetch();
        $this->load->view('backend/incident',$data);
        }
    else{
    ...
    public function Add_Incidentem() {
		if ($this->session->userdata('user_login_access') != False) {
			$this->load->model('Employee_model'); // Load Employee_model
			$data['enamevalue'] = $this->employee_model->GetAllEmployee(); // Call getEmployees() method
			$this->load->view('backend/add-incident', $data);
		} else {
			redirect(base_url(), 'refresh');
		}
	}
    ...
    public function add_Incident(){ 
        if($this->session->userdata('user_login_access') != False) { 
            ...
            $this->form_validation->set_rules('reporting_manager', 'reporting_manager', 'trim|required|min_length[1]|max_length[150]|xss_clean'); 
            $this->form_validation->set_rules('incident_date', 'incident_date', 'trim|required|min_length[1]|max_length[150]|xss_clean'); 
            $this->form_validation->set_rules('reporting_date', 'reporting_date', 'trim|required|min_length[1]|max_length[150]|xss_clean'); 
            $this->form_validation->set_rules('incident_description', 'incident_description', 'trim|required|min_length[1]|max_length[150]|xss_clean'); 
            ...
            if ($this->form_validation->run() == FALSE) { 
                ...
                if(empty($id)){ 
                    $success = $this->incident_model->Add_Incident($data); 
                    $this->session->set_flashdata('feedback','Successfully Added'); 
                    redirect('incident/Incident', 'refresh'); // Redirect to incident/Incident page
                } else { 
                    $success = $this->incident_model->Update_Incident($id,$data); 
    ...
    public function DeletIncident(){ 
		if($this->session->userdata('user_login_access') != False) {   
			$id= $this->input->get('D'); 
			$success = $this->incident_model->DeletIncident($id); 
        ...
        public function IncidentByID(){ 
			if($this->session->userdata('user_login_access') != False) {   
	  $id= $this->input->get('id'); 
	  $data['incident'] = $this->incident_model->GetIncidentValue($id); 
