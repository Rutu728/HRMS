...
class Employee extends CI_Controller {
	...
	public function Performance(){ 
		if($this->session->userdata('user_login_access') != False) { 
			$data['performance'] = $this->employee_model->performancefetch(); 
			$this->load->view('backend/performance',$data);  
			} 
		else{ 
			redirect(base_url() , 'refresh'); 
		}             
	...
	public function DeletPerformance(){ 
		if($this->session->userdata('user_login_access') != False) {   
			$id= $this->input->get('D'); 
			$success = $this->employee_model->DeletPerformance($id); 
			#echo "Successfully Deletd"; 
		...
		public function add_Performance(){ 
			if($this->session->userdata('user_login_access') != False) { 
				...
				$total_score = $this->input->post('total_score'); // Retrieve total_score from POST data 
		 
				// Validate input fields 
				$this->load->library('form_validation'); 
				$this->form_validation->set_error_delimiters(); 
				$this->form_validation->set_rules('attendance', 'attendance', 'trim|required|min_length[1]|max_length[150]|xss_clean'); 
				$this->form_validation->set_rules('quality', 'quality', 'trim|required|min_length[1]|max_length[150]|xss_clean'); 
				$this->form_validation->set_rules('productivity', 'productivity', 'trim|required|min_length[1]|max_length[150]|xss_clean'); 
				$this->form_validation->set_rules('behaviour', 'behaviour', 'trim|required|min_length[1]|max_length[150]|xss_clean'); 
				...
				if ($this->form_validation->run() == FALSE) { 
					...
					$data = array( 
						...
						'behaviour' => $behaviour, 
						'total_score' => $total_score // Include total_score in the data array 
					); 
		 
					// Insert or update the record 
					if(empty($id)){ 
						$success = $this->employee_model->Add_Performance($data); 
						$this->session->set_flashdata('feedback','Successfully Added'); 
						echo "Successfully Added"; 
					} else { 
						$success = $this->employee_model->Update_Performance($id,$data); 
						echo "Successfully Updated"; 
					} 
				} 
			} else { 
		...
		public function PerformanceByID(){ 
			if($this->session->userdata('user_login_access') != False) {   
	  $id= $this->input->get('id'); 
	  $data['performance'] = $this->employee_model->GetPerformanceValue($id); 
	  echo json_encode($data); 
			} 
		else{ 
	  redirect(base_url() , 'refresh'); 
        ...
        public function DeletResignation() {
            if($this->session->userdata('user_login_access') != false) {  
                $id = $this->input->get('D'); // Get the resign_id from the URL parameter
                $success = $this->employee_model->DeletResignation($id); // Call the model method with the correct parameter
                    redirect('employee/Resignation'); // Redirect after successful deletion
        ...
        public function ResignationByID(){
            if($this->session->userdata('user_login_access') != False) {  
            $id= $this->input->get('id');
            $data['resignation'] = $this->employee_model->GetResignValue($id);
            echo json_encode($data);
            }
        else{
            redirect(base_url() , 'refresh');
        ...
        public function Resignation(){
            if($this->session->userdata('user_login_access') != False) {
            $data['resignation'] = $this->employee_model->resignationfetch();
            $this->load->view('backend/resignation',$data); 
            }
        else{
            redirect(base_url() , 'refresh');
        }            
        ...
        public function add_Resignation(){
            ...
            $this->load->library('form_validation');
               $this->form_validation->set_error_delimiters();
            $this->form_validation->set_rules('notice_period', 'notice_period', 'trim|required|min_length[1]|max_length[150]|xss_clean');
            $this->form_validation->set_rules('resign_reason', 'resign_reason', 'trim|xss_clean');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if(empty($id)){
                    $success = $this->employee_model->Add_Resignation($data);
                    $this->session->set_flashdata('feedback','Successfully Added');
                    #redirect('employee/Disciplinary');
                    echo "Successfully Added";
                } else {
                    $success = $this->employee_model->Update_Resignation($em_id,$data);
                    #$this->session->set_flashdata('feedback','Successfully Updated');
                    #redirect("employee/view?I=" .base64_encode($em_id));
                    echo "Successfully Updated";
                }
